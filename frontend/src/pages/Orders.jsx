      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-4 text-sm">
          ...
          <div className="text-green-400">
            Confirmed:{" "}
            <span className="text-white">{stats.confirmed || 0}</span>
          </div>
          <div className="text-blue-400">
            Pickup: <span className="text-white">{stats.pickup || 0}</span>
          </div>
          <div className="text-red-400">
            Late: <span className="text-white">{stats.late || 0}</span>
          </div>
        </div>
        <button
      ...
      {orders.length === 0 ? (
        <div className="card p-8 text-center">
          ...
          <p className="text-white/60 mb-4">
            Start exploring our catalog to rent products
          </p>
          <button
            onClick={() => navigate("/catalog")}
            className="btn btn-primary"
              ...
              <div className="flex items-start justify-between">
                <div className="flex items-start space-x-4">
                  ...
                  <div className="w-16 h-16 bg-gradient-to-br from-primary/30 to-accent/30 rounded-lg overflow-hidden flex-shrink-0">
                    ...
                    order.productId.images.length > 0 ? (
                      <img
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-white/60 text-sm mt-1">
                      Order ID: {order.bookingId || order.id}
                    </div>
                    <div className="flex items-center space-x-4 mt-2 text-sm">
                      ...
                      <div>
                        ...
                        <span className="text-white ml-1">
                          {order.rentalDuration} {order.rentalType}
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <div className="text-white">
                      {formatCurrency(order.pricingSnapshot?.rentalAmount || 0)}
                    </div>
                  </div>
                  <div>
                    <span className="text-white/60">Deposit:</span>
                    <div className="text-white">
                      {formatCurrency(order.pricingSnapshot?.deposit || 0)}
                    </div>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <div className="text-white">
                      {order.productId?.ownerId?.firstName}{" "}
                      {order.productId?.ownerId?.lastName}
                    </div>
                  </div>
                </div>
              </div>
